...
for (Tr,Tc,fftw,lib) in ((:Float64,:Complex128,"fftw",libfftw),
    ...
    @eval function (::Type{r2rFFTWPlan{$Tr,T,inplace,N}})(X::StridedArray{$Tr,N},
                                                          Y::StridedArray{$Tr,N},
                                                          region, kinds, flags::Integer,
                                                          timelimit::Real) where {T,inplace,N}
        R = isa(region, Tuple) ? region : copy(region)
    ...
    @eval function (::Type{r2rFFTWPlan{$Tc,T,inplace,N}})(X::StridedArray{$Tc,N},
                                                          Y::StridedArray{$Tc,N},
                                                          region, kinds, flags::Integer,
                                                          timelimit::Real) where {T,inplace,N}
        R = isa(region, Tuple) ? region : copy(region)
